...
A Next.js app with no separate backend. Pages are static or server rendered and
deploy as one unit. There is no API service, no database and no auth unless a
feature adds one. Chosen for marketing sites, where SEO and page speed matter
more than live data.
...
The repository owner is the author of everything in it, whichever tool typed it.
...
This rule lives in `AGENTS.md` on purpose. Kept only in `CLAUDE.md` it reaches
Claude Code and not Codex, Copilot, Cursor, Gemini, Aider or OpenCode, so every
other tool asks again as though it had never been decided.
