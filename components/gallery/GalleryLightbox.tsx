            ...
            <div className="relative flex min-h-0 w-full max-w-[1200px] flex-1 self-center">
              ...
              */}
              {/*
                ...
                something real to resolve against. The photograph then fills
                that box and `object-contain` does the centring.
              */}
              <div
              ...
              >
                <Photo
                  ...
                  priority
                  /*
                   * The box is the frame, and `object-contain` fits the
                   * photograph inside it. So every photo reaches the full width
                   * or the full height, whichever comes first, on every screen.
                   *
                   * This replaced `h-auto max-h-full w-auto max-w-full`, which
                   * only ever *shrinks* a photograph to fit - it never grows one.
                   * With both axes auto the drawn size is the image's intrinsic
                   * size, so a photo filled the frame only when it happened to
                   * be bigger than it, and sat marooned in the middle when it
                   * was not.
                   *
                   * The trap is that "intrinsic size" here is not the file's
                   * pixels. For a `srcset` with `w` descriptors the browser
                   * divides the served pixels by the candidate's density, so
                   * `pink-floral-art` - a 741x1500 master - reported
                   * `naturalWidth` 368 behind the 828w candidate at a 412px
                   * viewport, and drew at 369x746 inside a 396x783 frame.
                   *
                   * Measured on the running page, worst case per device:
                   * 412px phone, photos 7 and 8 short on both axes; iPad mini,
                   * 2, 3, 7 and 8; iPad Pro, even photo 1 - the 2560px master -
                   * came out 922x860 in a 985x1226 frame. Only the 27" desktop
                   * was ever correct, because there the frame's height binds and
                   * everything clamps.
                   *
                   * It looked width-dependent and is not. At 375px the browser
                   * picks the 750w candidate, which lands the intrinsic width on
                   * 370 against a 359px frame - just over, so it clamps and
                   * fills. At 412px it picks 828w and lands on 369 against 396 -
                   * just under. A coincidence either way, not a breakpoint, and
                   * no breakpoint could have fixed it.
                   *
